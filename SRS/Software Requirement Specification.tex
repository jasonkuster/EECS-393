...
\subsection{Nextbus}
\subsubsection{Description and Priority}
A user will be presented with an active timer until campus buses arrive near them. Users can specify which bus route and which direction they want to view the times for at an expected time for at a specified stop. Priority: Very High
\subsubsection{Stimulus/Response Sequences}
\begin{description}\itemsep1pt
\item[Stimulus:] User opens Nextbus feature\\
\item[Response:] Nextbus feature is loaded presenting the user with a current prediction for their selection of route, direction, and stop. The current prediction is a active timer which predicts how many minutes will pass before a "greenie" headed in the requested direction on the requested route will pass the requested stop\\
...
~&~\\
Nextbus.SelectStop & The user can use a touchscreen to accurately make selections from a dropdown menu \\
~&~\\
Nextbus.SelectRoute & The user can use a touchscreen to accurately make selections from a dropdown menu \\
~&~\\
Nextbus.SelectDirection & The user can use a touchscreen to accurately make selections from a dropdown menu \\
~&~\\
\hline
\end{tabularx}
\end{table}
